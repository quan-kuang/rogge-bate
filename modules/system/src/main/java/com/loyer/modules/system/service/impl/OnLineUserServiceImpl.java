...
/**
 * @author kuangq
 * @title OnLineUserServiceImpl
 * @description 在线用户ServiceImpl
...
public class OnLineUserServiceImpl implements OnLineUserService {
    ...
    public void asyncSaveOnLineUser(HttpServletRequest httpServletRequest, String token, User user) {
        ...
        redisUtil.setValue(key, onLineUser, SystemConst.USER_EXPIRE_TIME);
    }
     ...
     * @return void
     * @author kuangq
     * @description 发送邮件登录通知
     * @date 2021-05-30 22:05
     */
    private void sendLoginInformMail(OnLineUser onLineUser) {
        Object value = redisUtil.getMap(PrefixConst.CONSTANT, PrefixConst.SEND_LOGIN_INFORM_MAIL_KEY);
        if (value == null) {
            ...
            redisUtil.setMap(PrefixConst.CONSTANT, PrefixConst.SEND_LOGIN_INFORM_MAIL_KEY, value);
        }
    ...
    public ApiResult selectOnLineUser(OnLineUser onLineUser) {
        List<OnLineUser> onLineUserList = redisUtil.fuzzyQuery(PrefixConst.ONLINE_USER);
        //校验结果集是否为空
    ...
    public ApiResult deleteOnLineUser(String... uuids) {
        ...
        for (String uuid : uuids) {
            redisUtil.delete(PrefixConst.LOGIN_USER + uuid);
            redisUtil.delete(PrefixConst.ONLINE_USER + uuid);
        }
