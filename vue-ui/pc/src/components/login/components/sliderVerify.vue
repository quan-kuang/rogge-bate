<!--suppress CssUnusedSymbol, RequiredAttributes, HtmlRequiredAltAttribute, JSUnresolvedVariable, JSUnusedGlobalSymbols -->
<template>
    <div class="slide-verify" :style="{width: canvasWidth + 'px'}">
        <!-- 图片加载遮蔽罩 -->
        <div :class="{'img-loading': isLoading}" :style="{height: canvasHeight + 'px'}" v-show="isLoading"/>
        <!-- 认证成功后的文字提示 -->
        <div class="success-hint" :style="{height: canvasHeight + 'px'}" v-show="verifySuccess">{{ successHint }}</div>
        <!--刷新按钮-->
        ...
        <template v-if="isFrontCheck">
            ...
            <!--阻塞块-->
            <canvas ref="block" :class="['slide-block', {'verify-fail shake-slow': verifyFail}]" :width="canvasWidth" :height="canvasHeight"/>
        </template>
        <!--后端生成-->
        <template v-else>
            <!--验证图片-->
            ...
            <!--阻塞块-->
            <img ref="block" :class="['slide-block', {'verify-fail shake-slow': verifyFail}]"/>
        </template>
        <!-- 滑动条 -->
        <div class="slider" :class="{'verify-active': verifyActive, 'verify-success': verifySuccess, 'verify-fail': verifyFail}">
            ...
            <div class="slider-box" :style="{width: sliderBoxWidth}">
                <!-- 按钮 -->
                <div :class="['slider-button', {'shake-slow': verifyFail}]" id="slider-button" :style="{left: sliderButtonLeft}">
                    <!-- 按钮图标 -->
                    <div class="slider-button-icon"/>
                </div>
            </div>
            <!--滑动条提示文字-->
            <span v-show="sliderButtonLeft===0" class="slider-hint">{{ sliderHint }}</span>
        </div>
    </div>
</template>
    ...
    methods: {
        ...
        getCaptcha() {
            ...
            const data = {
                ...
                blockWidth: self.blockWidth / 1.5,
        ...
        createImage(onload) {
            ...
            image.onload = onload;
            image.onerror = () => {
                image.src = require('../../../assets/img/bgImg.jpg');
            };
...
<style scoped>
    ...
    .refresh-icon {
        ...
        height: 35px;
        cursor: pointer;
        background: url("../../../assets/img/light.png") 0 -432px;
    ...
    .slider-button-icon {
        ...
        width: 15px;
        height: 13px;
        background: url("../../../assets/img/light.png") 0 -26px;
        background-size: 35px 470px
    }
    ...
    /*校验失败时的滑块的抖动效果*/
    .shake-slow {
        animation: shake-slow 0.5s 1 ease-in-out;
    }
